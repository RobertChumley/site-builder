...
let PopDownMenu = (props) => {
    ...
    let updateFilter = (e)=>{
        props.onFilterChanged(props, e.target.value);
    };
    return (
     <div style={{position: "absolute", marginLeft: "-8px",marginTop: "9px",zIndex:"1000", float: "left", width: "200px", height:"300px",backgroundColor: "white",border: "1px solid #DDD", display:(props.show ? "block":"none")}}>
            <div><input className={"form-control"} type={"text"} placeholder={"Search"} onChange={updateFilter.bind(this)}/></div>
...
class TableHeaderMenu extends Component{
    ...
    _handleFilter(menu_obj, value, e){
        let filter = {};
        filter[menu_obj.field.field_code] = value;
        this.service_manager.resolve(this.props.definition.table_name).emit("filter-updated",{filter: filter});
    }
    render(){
        return (
            <span>
                <a className={"nav-link"} onClick={this._showMenu.bind(this)}><DeHaze /></a>
                <PopDownMenu show={this.state.show} updateSort={this._handleSort.bind(this)} field={this.props.field} onFilterChanged={this._handleFilter.bind(this)}/>
            </span>);
    }
}
...
let ColumnFooterGroup = (params) =>{
    return (
        <tfoot>
            <tr><td/>{(params.fields || []).map((field, index)=>{
                if(field.show === false)
                    return;
                return <th key={"table-footer-" + field.field_name + "-" + index}>{field.field_name}</th>;})}</tr>
        </tfoot>
    );
};
...
export class TableMasterComponent extends Component{
    constructor(props){
        super();
        this.service_manager = ObjContainer.resolve("service_manager");
        this.state = {filter:{},rows: [],definition:(props.config || {fields:[]})};
    }
    async componentDidMount(){
        ...
        if(this.service_manager.hasType(this.props.config.table_name)) {
            ...
            this.service_manager.resolve(this.props.config.table_name).addListener("update-grid", async () => {
                this.data_def = await this.service_manager.resolve(this.props.config.table_name).get(this.state.filter);
            ...
            this.service_manager.resolve(this.props.config.table_name).addListener("filter-updated", async (filter) => {
                ...
                this.data_def = await this.service_manager.resolve(this.props.config.table_name).get(fullFilter);
                this.setState({
            ...
            this.setState({rows: this.data_def.data.data,
                paging: {count:this.data_def.data.count,page:this.data_def.data.page || 1, page_size: this.data_def.data.page_size},
                definition: this.data_def.schema_def,filter:filter});
        }
    }
    ...
    async _onDelete(row){
        ...
        await this.service_manager.resolve(this.state.definition.table_name).emit("update-grid");
    }

    render(){
        let clickFunc = this.props.rowClick || this._tableNav;
        return (
            <div>
                                ...
                                return (<tr key={"table-row-" + index}><DefaultActions ops={{onDelete: this._onDelete.bind(this,row)}} />{this.state.definition.fields.map((field, col_index)=>{
                                    ...
                                    if(row[field.field_code] instanceof Object){
                                        if(field.link)
                                            return (
                                                <td key={"table-row-" + index + '-' + col_index}>
                                                    <a className="nav-link" onClick={clickFunc.bind(this, field, row)}>{row[field.field_code].display || 'empty'}</a>
                                                </td>);
                                        else
                                        ...
                                        if(field.link)
                                            return (
                                                <td key={"table-row-" + index + '-' + col_index}>
                                                    <a className="nav-link" onClick={clickFunc.bind(this, field, row)} >{row[field.field_code] || 'empty'}</a>
                                                </td>);
                                        else
                                            return (<td  key={"table-row-" + index + '-' + col_index}>{((row[field.field_code] === undefined ||  row[field.field_code] === null) ? "" : row[field.field_code]).toString()}</td>);
                                    }
                                })}</tr>);
                            })}
                        </TableContent>
